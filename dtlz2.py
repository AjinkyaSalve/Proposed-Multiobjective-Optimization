...
# This code implements the DTLZ2 benchmark function evaluated in our study.
...
# --- DTLZ2 Objective Functions (true spherical formulation) ---
def dtlz2_g(x):
    """DTLZ2 auxiliary function"""
    xm = x[2:]
    return sum((xi - 0.5) ** 2 for xi in xm)

def dtlz2_f1(x):
    g = dtlz2_g(x)
    return (1 + g) * math.cos(x[0] * math.pi / 2) * math.cos(x[1] * math.pi / 2)

def dtlz2_f2(x):
    g = dtlz2_g(x)
    return (1 + g) * math.cos(x[0] * math.pi / 2) * math.sin(x[1] * math.pi / 2)

def dtlz2_f3(x):
    g = dtlz2_g(x)
    return (1 + g) * math.sin(x[0] * math.pi / 2)
...
# Main execution
num_variables = 12  # M-1 + k, with M=3 objectives and k=10 (standard DTLZ2 setting)
variable_ranges = [(0, 1)] * num_variables
objective_functions = [dtlz2_f1, dtlz2_f2, dtlz2_f3]
population_size = 100
max_iterations = 400
archive_size = 100
num_runs = 30
...
true_pf = problem.pareto_front(ref_dirs)
...
plt.title('Convergence of Performance Metrics for DTLZ2 Problem', fontsize=16, fontweight='bold', pad=12)
plt.xlabel('Number of Function Evaluations', fontsize=15, labelpad=8)
plt.ylabel('Metric Value', fontsize=15, labelpad=8)

# Places the legend cleanly OUTSIDE on the right side.
...
plt.tight_layout()
plt.savefig('dtlz2convg.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
print("Saved: dtlz2convg.png successfully.")

# ==================================================
# PLOT 2: True vs Obtained Pareto Front Validation (3D, DTLZ2 has 3 objectives)
# ==================================================
...
ax.scatter(obtained_pareto_front[:, 0], obtained_pareto_front[:, 1], obtained_pareto_front[:, 2],
           color='red', s=30, alpha=0.8, marker='o', label='Obtained Pareto Front', zorder=2)

plt.title('True vs. Obtained Pareto Front for DTLZ2', fontsize=16, fontweight='bold', pad=12)
...
plt.tight_layout()
plt.savefig('dtlz2paretofront.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
print("Saved: dtlz2paretofront.png successfully.")
