...
def g(x):
    ...
    if m_prime == 0:
        return 0
    return 1 + (9.0 / m_prime) * np.sum(x[2:])
    ...
    for it in range(1, max_iter + 1):
        ...
        for i in range(pop_size):
            ...
            if random.random() < 0.1:
                mutate_idx = random.randint(0, num_variables - 1)
                new_vars[mutate_idx] = max(0.0, min(1.0, new_vars[mutate_idx] + random.gauss(0, 0.05)))
...
def plot_combined_metrics(history):
    ...
    plt.title('Convergence of Performance Metrics for DTLZ7 Problem', fontsize=16, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    plt.savefig('dtlz7convg.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    print("Saved: dtlz7convg.png successfully.")

def plot_pareto_front(true_pf, final_front):
    """3D Pareto Plot formatted for DTLZ7 curve alignment"""
...
if __name__ == "__main__":
    ...
    print("--- Starting 30-Run Optimization for DTLZ7 ---")
