# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 17:04:17 2026
...
def multi_objective_rao(population_size, num_variables, variable_ranges, objective_functions, max_iterations, archive_size, minimization=True):
...
def zdt6_f1(x):
    return 1 - math.exp(-4 * x[0]) * (math.sin(6 * math.pi * x[0]) ** 2)

def zdt6_g(x):
    return 1 + 9 * (sum(x[1:]) / (len(x) - 1)) ** 0.25

def zdt6_f2(x):
    g = zdt6_g(x)
    return g * (1 - (zdt6_f1(x) / g) ** 2)
...
def calculate_coverage(true_pf, algorithm_data):
    ...
    true_pf_np = np.array(true_pf)
    algorithm_data_np = np.array(algorithm_data)

    max_vals = np.max(true_pf_np, axis=0)
    min_vals = np.min(true_pf_np, axis=0)
    range_vals = max_vals - min_vals
    range_vals[range_vals == 0] = 1

    true_pf_norm = (true_pf_np - min_vals) / range_vals
    algorithm_data_norm = (algorithm_data_np - min_vals) / range_vals

    min_distances = []
    for true_point in true_pf_norm:
        if len(algorithm_data_norm) > 0:
            distances = np.linalg.norm(algorithm_data_norm - true_point, axis=1)
...
def calculate_metrics(true_pf_data, algorithm_data, ref_point):
    ...
    if len(algorithm_data) > 0:
        ...
        if len(true_pf_data) > 0:
            ...
            ind_gd_plus = GDPlus(true_pf_data_np)
            ind_igd_plus = IGDPlus(true_pf_data_np)
            gd_plus_value = ind_gd_plus(algorithm_data_np)
            igd_plus_value = ind_igd_plus(algorithm_data_np)

            spread_value = calculate_spread(true_pf_data_np, algorithm_data_np)
            coverage_value = calculate_coverage(true_pf_data_np, algorithm_data_np)
...
objective_functions = [zdt6_f1, zdt6_f2]
population_size = 100
max_iterations = 400
archive_size = 100
num_runs = 30
...
all_runs_metrics_per_iter = {
    ...
    "Coverage": [[] for _ in range(max_iterations)]
}

final_run_metrics_values = {
    "GD": [], "GD+": [], "IGD": [], "IGD+": [],
    "HV": [], "Spacing": [], "Spread": [], "Coverage": []
}
run_times = []
start_time_total = time.time()

print("Starting Multi-Objective RAO Optimization...")

for run in range(num_runs):
    start_time_run = time.time()
    final_pareto_front, archives_per_iteration = multi_objective_rao(population_size, num_variables, variable_ranges, objective_functions, max_iterations, archive_size)
    ...
    for i, archive_at_iter in enumerate(archives_per_iteration):
        ...
        # Unpack all 8 values as calculate_metrics returns 8
        ...
        all_runs_metrics_per_iter["GD+"][i].append(gd_plus)
        all_runs_metrics_per_iter["IGD"][i].append(igd)
        all_runs_metrics_per_iter["IGD+"][i].append(igd_plus)
        ...
        all_runs_metrics_per_iter["Coverage"][i].append(coverage)

    final_archive_values = np.array([sol.objective_values for sol in final_pareto_front if sol.objective_values is not None])
    gd, gd_plus, igd, igd_plus, hv, spacing, coverage, spread_value = calculate_metrics(true_pf, final_archive_values, reference_point)
    final_run_metrics_values["GD"].append(gd)
    final_run_metrics_values["GD+"].append(gd_plus)
    final_run_metrics_values["IGD"].append(igd)
    final_run_metrics_values["IGD+"].append(igd_plus)
    ...
    final_run_metrics_values["Coverage"].append(coverage)
...
# ==========================================
# Expanded width to 10.5 to give the external legend its own dedicated space
...
plt.title('Convergence of Performance Metrics for ZDT6 Problem', fontsize=16, fontweight='bold', pad=12)
plt.xlabel('Number of Function Evaluations', fontsize=15, labelpad=8)
plt.ylabel('Metric Value', fontsize=15, labelpad=8)

# FIXED: Places the legend cleanly OUTSIDE on the right side.
# bbox_to_anchor=(1.02, 0.5) puts it 2% past the right axis border, centered vertically (0.5)
...
# CRITICAL FOR EXTERNAL LEGENDS: tight_layout ensures the outside box is included 
# in the saved image boundaries and doesn't get chopped off.
plt.tight_layout()
plt.savefig('zdt6convg.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
# FIXED: Changed marker sizes and layering order (zorder) so True Front markers 
# remain visible behind the Obtained Front points instead of clipping out at boundaries.
...
plt.title('True vs. Obtained Pareto Front for ZDT6', fontsize=16, fontweight='bold', pad=12)

# FIXED: Converted axis labels to proper math-formatted script ($f_1$, $f_2$) and bumped text size
...
plt.grid(False)
plt.tight_layout()
plt.savefig('zdt6pf.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()
print("Saved: Figure_Pareto_Front_Final.png successfully.")

# Statistics for final run metrics
print("\n--- Performance Metric Statistics (Final Archives) ---")
...
for metric_name, values in final_run_metrics_values.items():
    ...
    if valid_values:
        # For metrics where lower is better (GD, IGD, Spacing, Spread, Coverage)
        # For HV, higher is better
        if metric_name in ["GD", "GD+", "IGD", "IGD+", "Spacing", "Spread", "Coverage"]:
...
# Numerical Comparison of Plot End vs. Final Stats Mean (for Verification)
print("\n--- Numerical Comparison: Plot End Average vs. Final Statistics Mean (for Verification) ---")
# Use the limited set of metrics for this comparison too, for consistency
for metric_name in metrics_to_plot_on_single_graph.keys():
    plot_end_avg = avg_metrics_over_time[metric_name][-1] if avg_metrics_over_time[metric_name] else np.nan
