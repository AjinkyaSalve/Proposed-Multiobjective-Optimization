...
def calculate_coverage(true_pf, algorithm_data):
    ...
    true_pf_norm = normalize_front(true_pf)
    algorithm_data_norm = normalize_front(algorithm_data)
...
def calculate_metrics(true_pf_data, algorithm_data, ref_point):
    ...
    hv = HV(ref_point=ref_point)(normalize_front(algorithm_data))
...
def multi_objective_rao(pop_size, num_variables, var_ranges, max_iterations, archive_size, objective_functions,
    ...
    for it in range(1, max_iterations + 1):
        ...
        if track_convergence:
            ...
            normalized_front = normalize_front(front_values)
...
def plot_combined_metrics(history):
    """Combined metrics convergence plot with external legend style"""
    ...
    plt.title('Convergence of Performance Metrics for IDTLZ2 Problem', fontsize=16, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    plt.savefig('idtlz2convg.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    print("Saved: idtlz2convg.png successfully.")

def plot_pareto_front(true_pf, final_front):
    """3D Pareto Front Visualization styled for overlay alignment"""
    final_front_norm = normalize_front(final_front)
    true_pf_norm = normalize_front(true_pf)
    ...
    ax.scatter(true_pf_norm[:, 0], true_pf_norm[:, 1], true_pf_norm[:, 2], 
               color='blue', s=30, alpha=0.6, marker='o', label='True Pareto Front', zorder=1)

    
    ax.scatter(final_front_norm[:, 0], final_front_norm[:, 1], final_front_norm[:, 2], 
...
if __name__ == "__main__":
    ...
    print("--- Starting 30-Run Optimization for IDTLZ2 ---")
    for run in range(num_runs):
        ...
        normalized_front = normalize_front(front_values)
        gd, gd_plus, igd, igd_plus, hv, spacing, coverage, spread_val = calculate_metrics(true_pf, normalized_front, reference_point)
        ...
        print(f"[{now}] Run {run+1}: GD={gd:.8f}, GD+={gd_plus:.8f}, IGD={igd:.8f}, IGD+={igd_plus:.8f}, "
              f"HV={hv:.8f}, Spacing={spacing:.8f}, Coverage={coverage:.8f}, Spread={spread_val:.8f}, Time={run_time:.8f}s")
